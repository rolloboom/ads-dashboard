...
// ── Column map (indices match Google Sheet columns)
const C = {
...
export default function Dashboard() {
  ...
  // KPI — latest snapshot per campaign
  const kpiRows = useMemo(() => {
    const map = {};
    filtered.forEach(r => {
      const key = r[C.company] + "||" + r[C.campaign];
      if (!map[key] || String(r[C.date]) > String(map[key][C.date])) map[key] = r;
    });
    return Object.values(map);
  }, [filtered]);

  const kpi = useMemo(() => {
    const spendY   = kpiRows.reduce((s,r) => s + n(r[C.spendY]), 0);
    const spendT   = kpiRows.reduce((s,r) => s + n(r[C.spendT]), 0);
    const month    = kpiRows.reduce((s,r) => s + n(r[C.monthSpend]), 0);
    const downloads= kpiRows.reduce((s,r) => s + n(r[C.conv]), 0);
    const total    = kpiRows.length;
    const active   = kpiRows.filter(r => String(r[C.status]).includes("крутить")).length;
    const polIssue = kpiRows.filter(r => ni(r[C.policyN]) > 0).length;
    const clicks   = kpiRows.reduce((s,r) => s + ni(r[C.clicksY]), 0);
    const cpa      = downloads > 0 ? spendY / downloads : 0;
    return { spendY, spendT, month, downloads, total, active, polIssue, clicks, cpa };
  }, [kpiRows]);
  ...
  const COLS = [
    { label:"Дата",          col:C.date,      render: r => <span style={{color:"var(--muted2)"}}>{String(r[C.date]).slice(0,10)}</span> },
    { label:"Компанія",      col:C.company,   render: r => <span style={{fontWeight:600}}>{r[C.company]}</span> },
      ...
      <div style={S.page}>
        ...
        {error && (
          <div style={S.errorBox}>
            ⚠ {error}. Перевір API ключ у Vercel Environment Variables.
          </div>
        )}

        {/* ── KPI */}
        <div style={S.kpiGrid}>
          <KpiCard label="Витрати вчора"   value={"$" + fmt2(kpi.spendY)}   color="accent" sub={`${kpiRows.length} кампаній`} />
        ...
        <div style={S.filterBox}>
          <div style={S.filterRow}>
            <FGroup label="Від">
              <input style={S.inp} type="date" value={dateFrom} onChange={e=>{setDateFrom(e.target.value);setPage(1)}} />
            </FGroup>
            <FGroup label="До">
              <input style={S.inp} type="date" value={dateTo} onChange={e=>{setDateTo(e.target.value);setPage(1)}} />
            </FGroup>
            <FGroup label="Акаунт">
              <select style={S.sel} value={company} onChange={e=>{setCompany(e.target.value);setPage(1)}}>
                <option value="">Всі акаунти</option>
                {companies.map(c=><option key={c} value={c}>{c}</option>)}
              </select>
            </FGroup>
            <FGroup label="Статус">
              <select style={S.sel} value={status} onChange={e=>{setStatus(e.target.value);setPage(1)}}>
                <option value="">Всі статуси</option>
                {statuses.map(s=><option key={s} value={s}>{s}</option>)}
              </select>
            </FGroup>
            <FGroup label="Policy">
              <select style={S.sel} value={policyF} onChange={e=>{setPolicyF(e.target.value);setPage(1)}}>
            ...
            <FGroup label="Тип">
              <select style={S.sel} value={typeF} onChange={e=>{setTypeF(e.target.value);setPage(1)}}>
                <option value="">Всі типи</option>
                {types.map(t=><option key={t} value={t}>{t}</option>)}
              </select>
            </FGroup>
            <FGroup label="Домен">
              <select style={S.sel} value={domain} onChange={e=>{setDomain(e.target.value);setPage(1)}}>
                <option value="">Всі домени</option>
                {domains.map(d=><option key={d} value={d}>{d}</option>)}
              </select>
            </FGroup>
            <FGroup label="Пошук" wide>
              <input
        ...
        <div style={S.resultsBar}>
          <span style={{color:"var(--muted)", fontSize:12}}>
            {loading ? "Завантаження…" : `${filtered.length.toLocaleString("uk-UA")} рядків${filtered.length !== rows.length ? ` (з ${rows.length.toLocaleString("uk-UA")})` : ""}`}
          </span>
          <span style={{color:"var(--muted)", fontSize:11}}>↑↓ клік по заголовку — сортування</span>
        </div>
        ...
        {/* ── PAGINATION */}
        {totalPages > 1 && (
          <Pager page={page} total={totalPages} onPage={p=>{setPage(p);window.scrollTo({top:0,behavior:"smooth"})}} />
        )}
      </div>
    </>
  );
...
function FGroup({ label, children, wide }) {
  return (
...
function Ellipsis({ v, w, muted }) {
  const s = String(v || "—");
  return (
    <span title={s} style={{display:"block",maxWidth:w,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:muted?"var(--muted2)":undefined}}>
      {s}
    </span>
  );
}
...
function Badge({ v, color }) {
  const colors = { blue:"rgba(59,130,246,.15)", green:"rgba(16,185,129,.15)", yellow:"rgba(245,158,11,.15)", red:"rgba(239,68,68,.15)", muted:"rgba(100,116,139,.15)" };
  const texts  = { blue:"var(--blue)", green:"var(--green)", yellow:"var(--yellow)", red:"var(--red)", muted:"var(--muted)" };
  return (
    <span style={{background:colors[color]||colors.muted,color:texts[color]||texts.muted,padding:"2px 8px",borderRadius:20,fontSize:11,fontWeight:600,whiteSpace:"nowrap"}}>
      {v||"—"}
    </span>
  );
}
...
function StatusBadge({ v }) {
  ...
  else if (s.includes("модерації") || s.includes("Pending"))    color = "blue";
  return <Badge v={s} color={color} />;
}

function PolicyCell({ n, d, s }) {
  const count = parseInt(n) || 0;
  if (!count) return <span style={{color:"var(--green)"}}>✓ OK</span>;
  const isBad = String(d).includes("Дизапрув");
  return (
    <span title={String(d)} style={{color:isBad?"var(--red)":"var(--yellow)",fontWeight:600,cursor:"help"}}>
      ⚠ {count}
    </span>
  );
}
...
function Spinner() {
  return (
    <div style={{width:36,height:36,border:"3px solid var(--bg4)",borderTopColor:"var(--accent)",borderRadius:"50%",animation:"spin .7s linear infinite",margin:"0 auto"}} />
  );
}
...
function Pager({ page, total, onPage }) {
  ...
  return (
    <div style={S.pager}>
      ...
      {from > 1 && <><PBtn onClick={()=>onPage(1)}>1</PBtn>{from>2&&<span style={{color:"var(--muted)"}}>…</span>}</>}
      {pages.map(p=><PBtn key={p} active={p===page} onClick={()=>onPage(p)}>{p}</PBtn>)}
      {to < total && <><span style={{color:"var(--muted)"}}>…</span><PBtn onClick={()=>onPage(total)}>{total}</PBtn></>}
      <PBtn disabled={page===total} onClick={()=>onPage(page+1)}>›</PBtn>
      <span style={{fontSize:12,color:"var(--muted)"}}>Сторінка {page} з {total}</span>
    </div>
  );
}
...
const S = {
  ...
  btn: { background:"var(--accent)", color:"var(--bg)", border:"none", borderRadius:8, padding:"9px 18px", fontWeight:700, cursor:"pointer", fontSize:13, transition:"opacity .15s" },
  ...
  th: { background:"#0b0e16", color:"var(--accent)", fontSize:11, fontWeight:600, textTransform:"uppercase", letterSpacing:".5px", padding:"12px 12px", textAlign:"left", whiteSpace:"nowrap", position:"sticky", top:0, zIndex:2, borderBottom:"1px solid var(--border)", cursor:"pointer" },
  tr: { borderBottom:"1px solid var(--border)" },
  td: { padding:"10px 12px", fontSize:13, verticalAlign:"middle" },

  empty: { textAlign:"center", padding:"60px 20px", color:"var(--muted)" },

  pager: { display:"flex", alignItems:"center", gap:8, marginTop:16, justifyContent:"center", flexWrap:"wrap" },
};
